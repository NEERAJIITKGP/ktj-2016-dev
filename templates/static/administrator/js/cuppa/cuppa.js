...
// Load alert: cuppa.setContent({url:"js/cuppa/html/alert.php", name:"alert", preload:false, data:{title:"Title", message:"Message"}, duration:0.3});
// init
    var cuppa = { debug:false };
    var cu = cuppa;
    try{ cuppa.script_path = document.currentScript.src.replace("cuppa.js", ""); }catch(err ){};
    trace = function(value){ try{ console.log(value); }catch(err){ } };
...
// setContent
    cuppa.content = function(data){ return cuppa.setContent(data); }
    cuppa.setContent = function (data){
        ...
        if(data.preload == undefined) data.preload = false;
        ...
        */
		if(data.load){
            ...
            var loadObject = {};
                ...
                loadObject.success = function(result){
                    ...
                    if(data.preload){
                        ...
                        jQuery(objContent).css('opacity', 0);
                        if(data.position) jQuery(objContent).css("position", data.position);
                        TweenMax.to(objContent, data.duration, {delay:data.delay+0.1, alpha:1, ease:data.ease, onComplete:Execute_function});
                        jQuery(objContent).css('display', data.display);
                    }
                };
                loadObject.error = function(result){
                    jQuery(objContent).trigger("error");
                }
                //++ Load
            ...
            var contentToRemove;
            if(data.last){ contentToRemove = jQuery(data.name).last();
            }else if(data.first){ contentToRemove = jQuery(data.name).first();
            }else contentToRemove = jQuery(data.name);
			try{ 
    ...
    cuppa.blockade = function(data){
        ...
        */
		if(data.load){
            ...
            var blockadeToRemove;
            if(data.last){ blockadeToRemove = jQuery(data.name).last();
            }else if(data.first){ blockadeToRemove = jQuery(data.name).first();
            }else blockadeToRemove = jQuery(data.name);
			try{ 
...
// blockade screen
    cuppa.blockadeScreen = function(value){
        ...
        if(value) cuppa.blockade({name:"blockade_screen", uniqueClass:"blocakde_transparent"});
        else cuppa.blockade({load:false, name:".blockade_screen", delay:0.4});
    }
// noscroll
    cuppa.noscroll = function(value){
        if(value == undefined) value = true;
        if(value){
...
*/
	cuppa.charger = function(data){
		...
		if(data.load){
            ...
            var chargerToRemove;
            if(data.last){ chargerToRemove = jQuery(data.name).last();
            }else if(data.first){ chargerToRemove = jQuery(data.name).first();
            }else chargerToRemove = jQuery(data.name);
			try{ 
...
/* scroll
    Example: var scroll = new cuppa.scroll(".scroll .bar", ".scroll .track", "y");
    To add content to scroll: 
                 scroll.setContent(".menu_settings .buttons");
                          
*/
    cuppa.scroll = function(bar, track, axis, disableWheelPropagation, wheelDisplacement){        
        ...
        this.area = jQuery(this.bar).parent(); jQuery(this.area)[0].class_reference = jQuery(this.area)[0].scroll = this;
        this.axis = axis; if(!this.axis) this.axis = "y";
        this.positionScroll;
        this.positionTrack;
        this.wheelDisplacement = (wheelDisplacement) ? wheelDisplacement : 15;
        this.disableWheelPropagation = (disableWheelPropagation) ? disableWheelPropagation : false;
        this.recorder_position = 0;
        this.auto_hide = false;
        this.auto_size = true;
        this.blockade_scrolls = false;
        ...
        //++ onSize
            this.onSize = function(e, scroll){
                ...
                if(this.scroll.axis.search("y") != -1){
                    var dim_area = cuppa.dimentions(this.scroll.content.area);
                    var content_height = dim_area.scrollHeight;
                    var container_height = dim_area.height3;
                    if(this.scroll.content.area == "body") container_height = jQuery(window).height();
                    try{
                        var scrollHeight = jQuery(this.scroll.content.area).get(0).scrollHeight;
                        var contentHeight = jQuery(this.scroll.content.area).outerHeight();
                    }catch(err){ return }
                    if(content_height > container_height ){
                        jQuery(this.scroll.area).css({display:"block",opacity:1});
                        this.scroll.mouseWheelDisabled = false;
                    }else{
                        jQuery(this.scroll.area).css({display:"none",opacity:0});
                        this.scroll.mouseWheelDisabled = true;
                    }
                    if(this.scroll.auto_size){
                        var dim_scroll_area = cuppa.dimentions(this.scroll.area);
                        var barHeight = (container_height/content_height)*dim_scroll_area.height2;
                        if(barHeight < 20) barHeight = 20;
                        jQuery(this.scroll.bar).height(barHeight);
                    }
                }
                if(this.scroll.axis.search("x") != -1){
                    var scrollWidth = jQuery(this.scroll.content.area).get(0).scrollWidth;
                    var contentWidth = jQuery(this.scroll.content.area).outerWidth();
                    if(scrollWidth > contentWidth ){
                        jQuery(this.scroll.area).css("display", "block");
                        this.scroll.mouseWheelDisabled = false;
                    }else{
                        jQuery(this.scroll.area).css("display", "none");
                        this.scroll.mouseWheelDisabled = true;
                    }
                    if(this.scroll.auto_size){
                        var barWidth = (contentWidth/scrollWidth)*jQuery(this.scroll.area).width();
                        if(barWidth < 20) barWidth = 20;
                        jQuery(this.scroll.bar).width(barWidth);
                    }
                }
            } 
        //--
        //++ is touch
            if(cuppa.isTouch()){ jQuery(this.area).css("display","none"); }
        //--
        //++ Set content
            this.setContent = function(target, x, y, auto_hide, only_mousewheel_activated){
                if(auto_hide == undefined) auto_hide = true;
                this.auto_hide = auto_hide;
                var content = {}
                    content.target = target;
                    content.area = target;
                    content.y = (y == undefined) ? true : y;
                    content.x = (x == undefined) ? false : x;
                    content.only_mousewheel_activated = only_mousewheel_activated;
                this.content = content;
                this.contentConfiguration();
                if(auto_hide) this.autoHide();
                //++ is touch
                    if(cuppa.isTouch()){ jQuery(content.area).css("overflow","auto"); }
                //--
            }
        //--
        //++ Content data configuration
            this.contentConfiguration = function(){
                ...
                //++ mousewheel
                    if(this.axis.search("y") != -1){
                        cuppa.mousewheel(this.content.target, function(e){
                            var reference = e.data[0];
                            var porcent = cuppa.statusScrollPercent(reference.content.target);
                            try{ if(reference.blockade_scrolls) e.preventDefault(); }catch(err){ }
                            if(reference.mouseWheelDisabled) return;
                            
                            var delta = 0;
                                if(e.originalEvent.detail) delta = e.originalEvent.detail;
                                if(e.originalEvent.deltaY) delta = e.originalEvent.deltaY*-1;
                                if(e.originalEvent.wheelDelta) delta = e.originalEvent.wheelDelta*-1;
                            ...
                            jQuery(e.data[0].bar).trigger("wheel", [direction]);
                            jQuery(e.data[0].track).trigger("wheel", [direction]);
                            if(e.data[0].area) jQuery(e.data[0].area).trigger("wheel", [direction]);
                        }, [this]);
                    }else{
        ...
        //--
        //++ Hide de scroll when mouse rollout content
            this.rollOutHide = function(value){
                if(value == undefined) value = true;
                if(value){
                    jQuery(this.content.area).unbind("mouseenter", this.showBar).bind("mouseenter", [this], this.showBar);
                    jQuery(this.content.area).unbind("mouseleave", this.hideBar).bind("mouseleave", [this], this.hideBar);
                    jQuery(this.area).unbind("mouseenter", this.showBar).bind("mouseenter", [this], this.showBar);
                    jQuery(this.area).unbind("mouseleave", this.hideBar).bind("mouseleave", [this], this.hideBar);
                }else{
                    jQuery(this.content.area).unbind("mouseenter", this.showBar);
                    jQuery(this.content.area).unbind("mouseleave", this.hideBar);
                    jQuery(this.area).unbind("mouseenter", this.showBar);
                    jQuery(this.area).unbind("mouseleave", this.hideBar);
                    this.showBar();
                }
            }
            this.showBar = function(e){
                var ref = this;
                if(e) ref = e.data[0];
                TweenMax.to(ref.area, 0.4, {alpha:1, ease:Cubic.easeIn});
            }
            this.hideBar = function(e, delay){
                if(delay == undefined) delay = 0;
                var ref = this;
                if(e) ref = e.data[0];
                TweenMax.to(ref.area, 0.4, {delay:delay, alpha:0, ease:Cubic.easeIn});
            }
        //--
        //++ Auto move content
...
// Example: cuppa.moveContent(".content .list", ".content", false, true, 0, 0.5, 700, "linear", true);
	cuppa.moveContent = function(target, area, moveScrollX, moveScrollY, positionX, positionY, duration, ease, value_in_porcent, only_return_data){       
        ...
        if(moveScrollX && moveScrollY){
            TweenMax.to(area, duration, { scrollLeft:positionX, scrollTop:positionY, ease:ease, onUpdate:onProgress, onComplete:onComplete } );
        }else if(moveScrollX){
            TweenMax.to(area, duration, { scrollLeft:positionX, ease:ease, onUpdate:onProgress, onComplete:onComplete } );
        }else if(moveScrollY){
            TweenMax.to(area, duration, { scrollTop:positionY, ease:ease, onUpdate:onProgress, onComplete:onComplete } );
        }
        ...
        function onProgress(){
            ...
            jQuery(target).trigger("progress_content", [percent, target]);
        }
        function onComplete(){
            var percent = cuppa.statusScrollPercent(area);
            if(moveScrollY && !percent.y) return;
            if(moveScrollX && !percent.x) return;
            jQuery(target).trigger("complete", [percent, target]);
        }
        return data;
	};
...
*/
    cuppa.eventGroups = [];
    // Add Event listener
        cuppa.addEventListener = function(type, listener, object, groupName, data) {
            ...
            if(type == "ready"){
                ...
                jQuery(object).bind(type, data, listener);
            }
