...
/* Convert url string to object */
    cuppa.urlToObject = function(string, urlDecode){
        ...
        for(var i = 0; i < data.length; i++){
            ...
            try{
                if(urlDecode) item[1] =  jQuery.URLDecode(item[1]);
...
/* Encrypt / Decript 
    This method use AES encryption 
*/
    cuppa.encrypt = function(string, key){
        var result = jQuery.base64Encode(GibberishAES.enc(string, key).toString());
        return result;
    }
    cuppa.decrypt = function(string, key){
        var result =  GibberishAES.dec(jQuery.base64Decode(string), key);
        return result;
    }
/* Number format */    
